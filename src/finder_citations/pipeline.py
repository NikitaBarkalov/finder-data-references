...
def find_all(
    ...
    if pattern == re_doi:
        links = list(
            filter(
                lambda link: re.sub("/", "_", link.replace("https://doi.org/", "")) != filename[:-4].lower(), pdf_dois
            )
        )
        text_links = list(
            filter(
                lambda link: re.sub("/", "_", link.replace("https://doi.org/", "")) != filename[:-4].lower(), text_dois
            )
        )
...
def find_by_loc(filename: str, ordered_text: str, initial_text: str, loc_pattern: tuple, res_list: list[dict]) -> None:
    ...
    links = [
        re.sub("\\s+", "", link.group(1))
        for text in short_contexts
        for link in re.finditer(loc_pattern[1], text)
...
class FinderPipeline:
    def __init__(self):
        ...
        try:
            ...
            logger.info("spaCy NER model loaded; author extraction enabled.")
        except Exception:
            logger.warning("Failed to load spaCy NER model; author extraction will be disabled.")
            self.ner_model = None

    def process_pdf(self, pdf_path: str, progress_callback: Callable[..., None] | None = None) -> dict[str, Any]:
        ...
        df_dois = (
            df_citations[df_citations["dataset_id"].str.startswith("http")]
            .drop(columns=["start"])
            .reset_index(drop=True)
        )
        df_ids = df_citations[~df_citations["dataset_id"].str.startswith("http")].reset_index(drop=True)
        report(
            f"After deduplication: {len(df_dois)} unique DOIs and {len(df_ids)} unique IDs.", "Deduplication finished."
        )
        if not df_dois.empty:
            df_dois["context"] = df_dois["context"].apply(lambda contexts: ";\n".join(contexts))
            df_dois["context"] = df_dois["context"].apply(lambda cont: re.sub("<.+?>", "", cont))
        dang_ids = []
        if not df_ids.empty:
            report("Clustering IDs and identifying tables...", "Clustering IDs and identifying table contexts.")
            df_ids = (
                df_ids.explode(["context", "start"], ignore_index=True)
            ...
            df_ids["context"] = df_ids.apply(
                lambda row: (
                    row["context"]
                    if not isinstance(row["table"], str)
                    else row["context"] + f"{row['dataset_id']} inside the Table {row['table']}"
                ),
                axis=1,
            )
            ...
            df_ids = (
                df_ids.drop(columns=["start", "near_links_count", "cluster_type", "table"])
                .groupby(by=["article_id", "dataset_id"])
                .agg(list)
                .reset_index()
            )
            df_ids["context"] = df_ids["context"].apply(lambda cont: ";\n".join(cont))
            df_ids["context"] = df_ids["context"].apply(lambda cont: re.sub("<.+?>", "", cont))
        if not df_ids.empty:
            ...
            if not df_dang_ids.empty:
                ...
                verifications = self.classifier.verify_ids(
                    texts,
                    cits,
                    cancel_check=lambda delay=None, progress=None: (
                        progress_callback(None, delay, progress) if progress_callback else None
                    ),
                )
                df_dang_ids["is_valid"] = verifications
                df_dang_ids = df_dang_ids[df_dang_ids["is_valid"] == "Yes"].drop(columns=["is_valid"])
                report(
                    f"Successfully verified {len(df_dang_ids)} IDs using LLM.", "Ambiguous ID verification finished."
                )
            else:
                report(
                    "No ambiguous IDs found, skipping LLM verification.", "No ambiguous IDs; skipping LLM verification."
                )
        ...
        if not df_dois.empty:
            known_articles_mask = df_dois["dataset_id"].apply(
                lambda link: extract_prefix(link) in self.article_prefixes
            )
        ...
        if not df_ids.empty:
            ...
            df_ids["type"] = self.classifier.classify_ids(
                df_ids["context"].tolist(), df_ids["dataset_id"].tolist(), cancel_check=classification_cancel_check
            )
        ...
        if not df_ids.empty:
            for _, row in df_ids.iterrows():
                pattern_obj = (
                    row["pattern"][0]
                    if isinstance(row["pattern"], list) and len(row["pattern"]) > 0
                    else row["pattern"]
                )
                ...
                res_dict = {"citation": row["dataset_id"], "context": row["context"], "category": row["type"]}
                if url:
                    res_dict["url"] = url
