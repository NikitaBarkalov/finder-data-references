...
re_table = re.compile("(?<![A-Za-z])(t\\s*a\\s*b\\s*l\\s*e\\s*\\d+)(?![A-Za-z0-9])", re.IGNORECASE)
re_table_mark = re.compile("<(\\d+)>")
re_mark = re.compile("<.+>")
re_doi = re.compile("(?<![A-Za-z0-9])(1\\s*[01]\\s*\\.(\\s*\\d\\s*){1,9}\\s*/\\s*\\S{1,70})")
re_doi_prefix = re.compile("/(1[01]\\.\\d+)/")
re_alphafold = re.compile(
    "(?<![A-Za-z0-9])(AF\\s*\\-\\s*[A-Z0-9]+\\s*\\-\\s*F\\d+(\\s*\\-\\s*model\\s*\\-\\s*v\\d+)?)(?![A-Za-z0-9])"
)
re_arrayexpress = re.compile("(?<![A-Za-z0-9])(E\\s*\\-\\s*[A-Z]{4}\\s*\\-\\s*\\d+)(?![A-Za-z0-9])")
re_biomodels = re.compile("(?<![A-Za-z0-9])((((BIOMD)|(MODEL))\\d{10})|(BMID\\d{12}))")
re_bioproject = re.compile("(?<![A-Za-z0-9])(PRJ((NA)|(EB)|(DB)|(EA)|(DA)|(NZ)|(DG)|(NS)|(NE))\\d+)(?![A-Za-z0-9])")
re_biosample = re.compile("(?<![A-Za-z0-9])(SAM[NED][A-Z]?\\d+)(?![A-Za-z0-9])")
re_biostudies = re.compile("(?<![A-Za-z0-9])(S\\s*\\-\\s*[A-Z]{4}[\\-\\_A-Z]*\\d+)(?![A-Za-z0-9])")
re_cellosaurus = re.compile("(?<![A-Za-z0-9])(CVCL\\s*_\\s*[A-Z0-9]{4})(?![A-Za-z0-9])")
re_chembl = re.compile("(?<![A-Za-z0-9])(CHEMBL\\d+)(?![A-Za-z0-9])")
re_dbgap = re.compile("(?<![A-Za-z0-9])(phs[0-9]{6}(\\s*\\.\\s*v\\d+\\s*\\.\\s*p\\d+)?)(?![A-Za-z0-9])")
re_ebisc = re.compile("(?<![A-Za-z0-9])([A-Z]{2,6}\\s*i\\d{3}\\-\\s*[A-Z]{1,2})(?![A-Za-z0-9])")
re_efo = re.compile("(?<![A-Za-z0-9])(EFO\\s*_\\s*\\d{7})(?![A-Za-z0-9])")
re_ega = re.compile("(?<![A-Za-z0-9])(EGA[DSC]\\d{11})(?![A-Za-z0-9])")
re_emdb = re.compile("(?<![A-Za-z0-9])(EMD\\s*\\-\\s*\\d{4,5})(?![A-Za-z0-9])")
re_empiar = re.compile("(?<![A-Za-z0-9])(EMPIAR\\s*\\-\\s*\\d{5,})(?![A-Za-z0-9])")
re_geo = re.compile("(?<![A-Za-z0-9])(GSE\\d+)(?![A-Za-z0-9])")
re_gisaid = re.compile("(?<![A-Za-z0-9])(EPI\\s*(\\s*_\\s*ISL\\s*_\\s*)?\\d+)(?![A-Za-z0-9])")
re_hipsci = re.compile("(?<![A-Za-z0-9])(HPSI\\d{4}\\s*i\\s*\\-\\s*[a-z]+\\s*_\\s*\\d+)(?![A-Za-z0-9])")
re_hpa = re.compile("(?<![A-Za-z0-9])(((HPA)|(CAB))\\d{6})(?![A-Za-z0-9])")
re_igsr = re.compile("(?<![A-Za-z0-9])(((GM)|(NA)|(HG))\\d{5})(?![A-Za-z0-9])")
re_intact = re.compile("(?<![A-Za-z0-9])(EBI\\s*\\-\\s*[0-9]+)(?![A-Za-z0-9])")
re_interpro = re.compile("(?<![A-Za-z0-9])(IPR\\d{6})(?![A-Za-z0-9])")
re_metabolights = re.compile("(?<![A-Za-z0-9])((MTBLS\\d+))(?![A-Za-z0-9])")
re_mint = re.compile("(?<![A-Za-z0-9])(((MINT)|(IM))\\s*\\-\\s*\\d{1,7})(?![A-Za-z0-9])")
re_nct = re.compile("(?<![A-Za-z0-9])(NCT\\d{8})(?![A-Za-z0-9])")
re_pfam = re.compile("(?<![A-Za-z0-9])(PF\\d{5})(?![A-Za-z0-9])")
re_pxd = re.compile("(?<![A-Za-z0-9])(PXD\\d{6})(?![A-Za-z0-9])")
re_reactome = re.compile(
    "(?<![A-Za-z0-9])((R\\s*\\-\\s*[A-Z]{3}\\s*\\-\\s*\\d+(\\s*\\-\\s*\\d+)?(\\s*\\.\\s*\\d+)?)|(REACT\\s*_\\s*\\d+(\\s*\\.\\s*\\d+)?))(?![A-Za-z0-9])"
)
re_refseq = re.compile("(?<![A-Za-z0-9])(((NC)|(NM))\\s*_\\s*\\d+(\\s*\\.\\s*\\d+)?)(?![A-Za-z0-9])")
re_rfam = re.compile("(?<![A-Za-z0-9])(RF\\d{5})(?![A-Za-z0-9])")
re_rnacentral = re.compile("(?<![A-Za-z0-9])(URS[0-9A-F]{10}(\\s*\\_\\s*\\d+)?)(?![A-Za-z0-9])")
re_sra = re.compile("(?<![A-Za-z0-9])(([SE]R[PRX]\\d{6,}))(?![A-Za-z0-9])")
re_treefam = re.compile("(?<![A-Za-z0-9])(TF\\d{6})(?![A-Za-z0-9])")
re_uniparc = re.compile("(?<![A-Za-z0-9])(UPI[A-F0-9]{10})(?![A-Za-z0-9])")
ID_PATTERNS = [
    re_refseq,
    re_gisaid,
    re_arrayexpress,
...
re_pdb_loc = re.compile("(?<![A-Za-z0-9])(pdb)(?![A-Za-z0-9])", re.IGNORECASE)
re_pdb = re.compile("(?<![A-Za-z0-9])([0-9]((([A-Z0-9]{3})|([a-z0-9]{3}))))(?![A-Za-z0-9])")
re_gen_loc = re.compile(
    "(?<![A-Za-z0-9])((g\\s*e\\s*n\\s*b\\s*a\\s*n\\s*k)|(a\\s*c\\s*c\\s*e\\s*s\\s*s\\s*i\\s*o\\s*n)|(acc\\s*(\\.)?))(?![A-Za-z0-9])",
    re.IGNORECASE,
)
re_gen = re.compile("(?<![A-Za-z0-9])([A-Z]{1,2}[0-9]{5,})(?![A-Za-z0-9])")
...
def make_local_regex(link: str, special_chars: str = "^$.|?*+()[]{}") -> re.Pattern:
    regex = "\\s*".join(char if char not in special_chars else "\\" + char for char in link)
...
def doi_correct(doi_cit: str) -> str:
    while doi_cit and (doi_cit[-1] in ".,;:!?\"'/" or not pair_chars(doi_cit)):
        doi_cit = doi_cit[:-1]
    doi_cit = re.sub("[\\-\\‐\\-\\‒\\–\\—\\―]", "-", doi_cit)
    return "https://doi.org/" + re.sub("\\s+", "", doi_cit).lower()
...
def validate_authors(authors: list[str]) -> str:
    if len(authors) == 0:
        return "Not found"
    corrected_authors = list(map(lambda author: re.sub("[^A-Za-z\\.\\s\\-\\|;]", "", author).strip(), authors))
    filtered_by_start_size = list(
        map(
            lambda author: " ".join(
                [name_part for name_part in author.split(" ") if len(name_part) > 0 and name_part[0].isupper()]
            ),
            corrected_authors,
        )
    )
    filtered_authors_by_length = list(
        filter(
            lambda name: 3 <= sum([char.isalpha() for char in name]) and len(name.split()) >= 1, filtered_by_start_size
        )
    )
