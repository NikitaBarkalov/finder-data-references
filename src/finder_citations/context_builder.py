...
from .extractors import doi_correct, make_local_regex, pair_chars, re_doi, re_gen, re_pdb
...
def mark_blocks(
    ...
    loc_patterns: list[tuple[re.Pattern, re.Pattern, int]],
    ...
    for loc_pat in loc_patterns:
        ...
        found = [
            ...
            for link in re.finditer(loc_pat[1], text)
...
def search_context(
    text: str, pattern: re.Pattern, cont_size: int = 300, min_batch_size: int = 50
...
def extract_doi_by_text(text: str, pattern: re.Pattern = re_doi) -> list[str]:
    ...
    for pos in doi_positions:
        ...
        for i in range(len(nearest_words)):
            ...
            not_broken_line = (
                (word.islower() or word.isupper())
                and link[-1] != ")"
                and any([char.isalpha() for char in word])
                and any([char in signs for char in word])
            )
            ...
            without_digits_suffix = (
                all([not char.isdigit() for char in link.split("/")[-1]])
                if len(link.split("/")[-1].strip()) > 0
                else False
            )
        ...
        trunc_chars = "@&=+$,"
        end_trunc_chars = "-‐–—/"
        without_stranges = all(
            [char not in link if len(link) > 0 and char != link.strip()[-1] else True for char in trunc_chars]
        )
...
def cluster_type_identify(
    df: pd.DataFrame, article: str, edge_threshold: int = 2, inner_threshold: int = 3
) -> pd.DataFrame:
        ...
        if i == df_art.index[0]:
            try:
                df.loc[i, "cluster_type"] = (
                    "Start" if _count(i) >= edge_threshold and _count(i + 1) >= inner_threshold else "Outer"
                )
