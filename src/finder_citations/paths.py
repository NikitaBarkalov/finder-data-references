...
def resolve_prefixes_path() -> str:
    """Locate prefixes.csv across Docker, editable, and source layouts."""
    candidates = []
    ...
    # Docker / process CWD (WORKDIR /app)
    candidates.append(os.path.join(os.getcwd(), "input_data", "prefixes.csv"))

    # Repo root when running from source: src/finder_citations/extractors.py -> ../../..
    ...
    # Fallback next to the installed package (if data is ever packaged alongside)
