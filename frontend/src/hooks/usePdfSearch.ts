...
export function usePdfSearch({
  ...
  const handleFindCitation = useCallback(
    (citationText: string) => {
      ...
      if (container) {
        ...
        document
          .querySelectorAll('.static-pdf-overlay.search-active')
  ...
  const handleSearch = useCallback(
    (text: string, forward: boolean = true, isTyping: boolean = false) => {
      if (!text) {
        setMatchCount(0);
        setCurrentMatch(0);
        setHighlightRects([]);
      ...
      let found = (
        window as unknown as {
          find: (...args: unknown[]) => boolean;
        }
      ).find(text, false, !forward, true, false, false, false);
      ...
      if (found && matchCount > 0) {
        setCurrentMatch((prev) => {
          ...
          return forward ? (prev < matchCount ? prev + 1 : 1) : prev > 1 ? prev - 1 : matchCount;
        });
      }
    ...
    [pdfContainerRef, matchCount, setHighlightRects],
  );
  return {
    searchText,
    setSearchText,
