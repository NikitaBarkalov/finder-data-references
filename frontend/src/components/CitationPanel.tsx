...
export function CitationPanel({
  ...
  const toggleHide = useCallback(
    (cit: string) => setHiddenCitations((prev) => ({ ...prev, [cit]: !prev[cit] })),
    [setHiddenCitations],
  );
  return (
        ...
        <div style={{ flex: '1 1 auto', minWidth: '350px' }}>
          ...
          >
            <div>
              ...
              <p style={{ margin: 0, marginTop: '0.2rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                AI-powered Data Reference Extractor & Classificator
              </p>
            </div>
            ...
            {pipelineStatus === 'loading' && (
              <div style={{ display: 'flex', gap: '0.5rem', marginLeft: 'auto' }}>
                <button
                  ...
                  onMouseOver={(e) => {
                    e.currentTarget.style.background = isExtractionPaused
                      ? 'rgba(34, 197, 94, 0.2)'
                      : 'rgba(234, 179, 8, 0.2)';
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', position: 'relative' }}>
                  ...
                  >
                    ...
                    {isDownloadingPdf ? (
                      isDownloadPaused ? (
                        <span>
                          Paused<span className="animated-ellipsis"></span>
                        </span>
                      ) : (
                        'Generating...'
                      )
          ...
          {pdfFilename && (
            <div style={{ marginTop: '0.25rem', fontSize: '0.85rem' }}>
              ...
              {pdfFilename.startsWith('10.') ? (
                ...
                <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{pdfFilename}</span>
              )}
            </div>
          )}
          ...
          {pipelineStatus === 'results' && results && (
            ...
            >
              <span
                style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500, marginBottom: '0.1rem' }}
              >
                HIGHLIGHT IN PDF:
              </span>
                ...
                {Object.keys(visibleCategories).map((cat) => (
                  <label
                    ...
                    style={{
                      ...
                      background: visibleCategories[cat] ? 'var(--bg-secondary)' : 'transparent',
                      padding: '0.25rem 0.5rem',
                      borderRadius: '4px',
                      border: '1px solid var(--border-color)',
                      transition: 'all 0.2s',
                      opacity: visibleCategories[cat] && !isCachedFile ? 1 : 0.4,
                    }}
                  >
                    <input
                      type="checkbox"
        ...
        {pipelineStatus === 'results' && results && (
          <div className="results-container results-entrance">
            <div className="categories-wrapper">
              <CategorySection
                title="PRIMARY DOI"
                citations={grouped.primaryDoi}
                badgeClass="primary-doi"
                onSearch={onFindCitation}
              ...
              <CategorySection
                title="SECONDARY DOI"
                citations={grouped.secondaryDoi}
                badgeClass="secondary-doi"
                onSearch={onFindCitation}
                activeSearch={activeCitationSearch}
                counts={citationCounts}
                hiddenCitations={hiddenCitations}
              ...
              <CategorySection
                title="PRIMARY ID"
                citations={grouped.primaryId}
                badgeClass="primary-id"
                onSearch={onFindCitation}
                activeSearch={activeCitationSearch}
                counts={citationCounts}
                hiddenCitations={hiddenCitations}
              ...
              <CategorySection
                title="SECONDARY ID"
                citations={grouped.secondaryId}
                badgeClass="secondary-id"
                onSearch={onFindCitation}
                activeSearch={activeCitationSearch}
                counts={citationCounts}
                hiddenCitations={hiddenCitations}
