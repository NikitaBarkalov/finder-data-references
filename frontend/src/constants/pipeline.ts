...
export function mapProgressMessage(msgStr: string): {
  ...
  if (msg.includes('raw citations before deduplication')) {
    ...
    return {
      stepIndex: 2,
      detail: match
        ? { index: 2, text: `${match[1]} raw references found` }
        : { index: 2, text: 'Extracting references' },
    };
  }
  ...
  if (msg.includes('identified') && msg.includes('prefix filter')) {
    ...
    return {
      stepIndex: 5,
      detail: match ? { index: 5, text: `${match[1]} items passed via prefix match`, append: true } : undefined,
    };
  }
