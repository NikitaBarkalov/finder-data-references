...
describe('citations utils', () => {
  it('maps citations to the expected category keys', () => {
    expect(getCategoryKey(makeCitation({ citation: 'https://doi.org/10.1/x', category: 'Primary' }))).toBe(
      'PRIMARY DOI',
    );
    expect(getCategoryKey(makeCitation({ citation: 'GSE12345', category: 'Primary' }))).toBe('PRIMARY ID');
    expect(getCategoryKey(makeCitation({ citation: 'https://doi.org/10.1/x', category: 'Secondary' }))).toBe(
      'SECONDARY DOI',
    );
  ...
  it('returns the configured category list and defaults', () => {
    expect(CATEGORY_KEYS).toEqual(['PRIMARY DOI', 'SECONDARY DOI', 'PRIMARY ID', 'SECONDARY ID', 'ARTICLE']);
